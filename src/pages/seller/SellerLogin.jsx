...
const SellerLogin = () => {
  ...
  useEffect(() => {
    ...
    const savedEmail = localStorage.getItem('seller_login_email');
    if (savedEmail) {
      setFormData(prev => ({
        ...prev,
        email: savedEmail,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (formData.rememberMe) {
        localStorage.setItem('seller_login_email', formData.email);
        localStorage.setItem('seller_logged_in', 'true');
        localStorage.setItem('seller_user', userData);
        localStorage.setItem('seller_token', token);
      } else {
        localStorage.removeItem('seller_login_email');
        sessionStorage.setItem('seller_logged_in', 'true');
        sessionStorage.setItem('seller_user', userData);
        ...
        <section className="relative z-10 max-w-7xl mx-auto px-6 py-12 md:py-20 grid grid-cols-1 lg:grid-cols-2 gap-10 items-center min-h-[calc(100vh-180px)]">
          <div className="reveal">
            <h1 className="text-3xl md:text-5xl font-extrabold leading-tight">
              Seller Login<span className="text-gray-900">.</span>
            </h1>
            <p className="mt-3 md:mt-4 text-gray-600 max-w-xl">
              Access your seller dashboard to manage products, orders, and payouts with ease.
            </p>
            <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm text-gray-700">
              <div className="flex items-center gap-2">
                <span className="text-rose-500">•</span> Analytics dashboard
              </div>
              <div className="flex items-center gap-2">
                <span className="text-amber-500">•</span> Fast payouts
              </div>
              <div className="hidden sm:flex items-center gap-2">
                <span className="text-rose-500">•</span> Easy product uploads
              </div>
            </div>
          </div>
