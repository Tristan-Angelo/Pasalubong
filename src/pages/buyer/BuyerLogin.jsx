...
import { buyerLogin } from '../../utils/api';
...
const BuyerLogin = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Call backend API
      const response = await buyerLogin({
      ...
      if (response.token) {
        if (formData.rememberMe) {
          ...
          sessionStorage.setItem('buyer_token', response.token);
        }
      }
      ...
      if (formData.rememberMe) {
        localStorage.setItem('buyer_email', formData.email);
        localStorage.setItem('buyer_logged_in', 'true');
        localStorage.setItem('buyer_user', userData);
      } else {
        sessionStorage.setItem('buyer_logged_in', 'true');
        ...
        <section className="relative z-10 max-w-7xl mx-auto px-6 py-12 md:py-20 grid grid-cols-1 lg:grid-cols-2 gap-10 items-center min-h-[calc(100vh-180px)]">
          <div className="reveal">
            ...
            <p className="mt-3 md:mt-4 text-gray-600 max-w-xl">
              Sign in to continue discovering and sending authentic pasalubong from Carigara & Barugo.
            </p>
            <div className="mt-6 flex items-center gap-6 text-sm text-gray-700">
              <div className="flex items-center gap-2">
                <span className="text-rose-500">•</span> Secure login
              </div>
              <div className="flex items-center gap-2">
                <span className="text-amber-500">•</span> Fast checkout
              </div>
